...
usuarios = {
    ...
    'abimael': {'password':  generate_password_hash("user123"), 'rol': 'User'}
    }
print(f'Usuarios en "sistema" {usuarios}')
...
@login_required
def home():
    rol = usuarios[current_user.id]['rol']
    return render_template('home.html.jinja2', nombre=current_user.id, rol=rol)
...
def login():
    ...
    if request.method == 'POST':
        ...
        return render_template("error.html.jinja2", error_code=401, error_message="Invalid Credentials (error on username or password)!"), 401
